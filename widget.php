...
class kol_email_form extends WP_Widget {
	...
	public function update( $new, $val ) {
		...
		foreach ( array( 'form_fields_name', 'blog_only' ) as $check_field )
		...
		<?php else : ?>
			...
			<?php else : ?>
				...
				<div id="<?php echo $this->get_field_id( 'kol-email' ); ?>-form" style="display: <?php echo $val['list'] ? 'block' : 'none'; ?>">
					...
					<p>
						<input id="<?php echo $this->get_field_id( 'blog_only' ); ?>" type="checkbox" name="<?php echo $this->get_field_name( 'blog_only' ); ?>" value="1" <?php checked( $val['blog_only'] ); ?> />

						<label for="<?php echo $this->get_field_id( 'blog_only' ); ?>"><?php echo $kol_email->strings['blog_only']; ?></label>
					</p>
